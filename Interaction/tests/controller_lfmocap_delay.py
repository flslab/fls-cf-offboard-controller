...
class LFMoCapDelayController:
    ...
    def _send_setpoint_loop_with_arrival(self, x: float, y: float, z: float,
        ...
        """
        Send position setpoints for ``duration`` seconds.

        Simultaneously polls own Vicon Y position for arrival (within
        ``arrive_tol`` of ``target_y``).

        Returns the Unix timestamp of first arrival detection, or
        ``time.time()`` at loop-end if arrival was never reached.
        """
    ...
    def _hover_and_detect_leader(self, hover_x: float, hover_y: float,
                                  ...
                                  timeout: float) -> float:
        """
        Send hover setpoints while polling KF-estimated leader Y-velocity.
