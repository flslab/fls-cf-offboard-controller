"""Exercise opt-in CLI and mission scoping without importing the controller."""
import argparse
import ast
...
class AdaptiveCalibrationCliTests(unittest.TestCase):
    def args(self, **changes):
        values = dict(calibrate=True, adaptive_braking_calibration=False,
    ...
    def test_flag_defaults_off_and_valid_logged_calibration_parses(self):
        defaults = parse_controller_args([])
        self.assertFalse(defaults.adaptive_braking_calibration)
    ...
    def test_cli_rejects_opt_in_without_exclusive_calibration(self):
        for modes in ([], ['--interaction'], ['--braking-test'],
                      ['--calibrate', '--interaction'], ['--calibrate', '--braking-test']):
            with self.subTest(modes=modes), redirect_stderr(io.StringIO()), self.assertRaises(SystemExit):
                parse_controller_args([*modes, '--adaptive-braking-calibration',
                                       '--log', '--smooth-controller-rate', '100'])
    ...
    def test_unselected_mode_does_not_copy_or_change_mission(self):
        ...
        self.assertIs(factory.call_args.args[3], mission)
