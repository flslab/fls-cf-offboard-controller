...
class SCurvePreflightTests(unittest.TestCase):
    ...
    def test_unused_acceleration_residual_is_explicitly_reset_if_supported(self):
        ctrl=self.velocity_controller();p=self.modern_velocity_params()
        p.toc.toc['hlCommander']['pRelCompB']=None;p.replies['hlCommander.pRelCompB']='0'
    ...
    def test_acceleration_residual_requires_capability_before_any_write(self):
        ...
        for key,value in ctrl._firmware_analytic_expected.items():
            if key=='hlCommander.pRelCompB':continue
            group,name=key.split('.');p.toc.toc.setdefault(group,{})[name]=None;p.replies[key]=str(value)
        with self.assertRaisesRegex(RuntimeError,'pRelCompB'):ctrl._setup_firmware_auto_brake_params()
        p.set_value.assert_not_called()
