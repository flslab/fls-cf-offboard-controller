...
def run(uri, duration_s, delivery_gate_hz):
    ...
    def on_data(timestamp, data, _logconf):
        ...
        with lock:
            received.append((host_time, int(data['contactImu.epoch'])))
        first_packet.set()
    ...
    passed = rate_hz >= delivery_gate_hz and not missed_events
    ...
    print(
        ...
        f'max_epoch_gap={max(missed_events, default=1)} ms'
    )
    print(
