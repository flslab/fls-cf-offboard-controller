...
def take_off_simple(scf):
    ...
    time.sleep(1)
    commander.go_to(0.1, 0.1, args.takeoff_altitude, 0, 0.5, relative=False)
    time.sleep(args.t)
...
def set_pid_values(scf, propeller_size=None, with_cage=False):
    ...
    if propeller_size == 2:
        ...
        cf.param.set_value('posCtlPid.zKp', '2.5')
        ...
        cf.param.set_value('velCtlPid.vzKd', '0.005')
        #
        ...
        while self._stay_open:
            ...
            for name, obj in mc.rigidBodies.items():
                if name == self.body_name:
                    pos = obj.position
                    if self.on_pose:
                        self.on_pose([pos[0], pos[1], pos[2] - 0.04, obj.rotation])
